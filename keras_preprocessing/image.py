...
class ImageDataGenerator(object):
    ...
    ```python
    df=read_csv("./train.csv") #Reading CSV file as pandas dataframe
    ...
    def flow_from_dataframe(self, dataframe, directory,
        ...
        # Returns
            ...
            of images with shape `(batch_size, *target_size, channels)`
             and `y` is a numpy array of corresponding labels.
        """
        return ImageFileIterator(self, dataframe, directory,
...
def _list_valid_filenames_in_directory(directory, white_list_formats, split,
    ...
    # Arguments
        ...
        follow_links: boolean.
        df: True,if used along with flow_from_dataframe
    # Returns
        ...
        if dataframe is None:
            ...
            if class_mode not in ["other", "input"]:
                if has_ext:
                    ...
                    temp_df=temp_df[temp_df[x_col].isin(filenames)]
                else:
                    filenames_without_ext = [f[:-1 * (len(f.split(".")[-1]) + 1)]
                                             for f in filenames]
                    temp_df = pd.DataFrame({x_col: filenames_without_ext},dtype=str)
                    temp_df = self.df.merge(temp_df, how='right', on=x_col)
                    temp_df=temp_df[temp_df[x_col].isin(filenames_without_ext)]
